...
**Version 5.2 — 2026-08-14.** This document describes the pipeline **as it currently stands**. It is a reference, not a change log: anything removed from the code is removed from here.
...
- **\[Technical\]**  
  - *How it works:* Recursively scans clips\_dir and builds a pool of annotatable frames (those with enough preceding frames for the motion window). Already-annotated frames are excluded via the AnnotationIndex. The tool supports four navigation modes selectable at any time from a **Source** button in the control bar: **(1) Random** — each save/skip draws a new random unannotated frame; exits when the pool is empty. This is the launch mode *unless* `mined_frames/mining_targets.csv` exists and still has pending rows, in which case the tool opens directly in Mined-frames mode on the first of them. The choice is made before the video capture is opened, so no random 4K clip is decoded over the mount only to be discarded; the Source button switches away at any time. **(2) Frame per frame** — stay inside the current video and advance by a fixed `seq_step` frames on every save/skip. The step is asked for when the mode is entered, and the dialog states the equivalent cadence at the clip's fps (1 = every frame, fps/2 = 2 fps, fps = 1 fps). Unlike Random it deliberately does **not** skip already-annotated frames: landing on one reloads its boxes for review, which is what continuous-stretch work needs. At the end of the clip it says so and stops advancing rather than jumping to another video. Pick the starting point with the seek bar (or arrive via Random/CSV) before switching the mode on. **(3) CSV time-codes** — load a CSV listing specific frames of interest; each save/skip then advances through those frames *in CSV order* (revisiting already-annotated frames is allowed, e.g. to verify a behaviour), automatically switching video as needed; when the list is exhausted the tool reverts to Random mode. The CSV may target many videos: a *video* column (video\_filename / video / filename) is matched against the pool on the filename stem (case-insensitive), and a *time* column (frame / timecode / time / start\_frame) is read either as an integer frame index or as an mm:ss / hh:mm:ss time-code converted via the video fps. Out-of-range frames are clamped; duplicates are dropped while preserving order. **(4) Mined frames** — walk the target list produced by `BehaveAI_mine_frames.py` (§3.1A), i.e. the frames on which the pipeline is visibly struggling. Mechanically identical to CSV mode (same parser, same ordered walk, same automatic video switching) and differing only in three respects: it defaults to `<output\_dir>/mining\_targets.csv` without a file dialog when that file exists, the optional `reason`/`detail` columns are shown in the title bar so the annotator knows what to look at, and the end-of-list message points back at the miner rather than simply reverting to Random. A plain time-code CSV gets the same treatment for its `behaviour` column. The Source button label shows the active mode and its progress (Source: Random / Source: Frame+15 / Source: CSV (i/N) / Source: Mining (i/N)). CSV files live in the project's timecodes/ folder; an example\_timecodes.csv template is created there on first use.  
  - *Purpose:* Spread annotation uniformly at random across all clips to avoid temporal bias (Random); label a continuous stretch of one clip at a controlled cadence, which is what behaviours only legible over time (Kick, Rear, Strike, Balk) and any per-clip dense pass require (Frame per frame); steer annotation toward specific behaviours/events recorded elsewhere — e.g. tracker outputs or hand-made event lists (CSV time-codes); or spend the annotation budget where the models are failing rather than where the phenomenon is common (Mined frames).  
  ...
  - *Implementation:* build\_frame\_pool(), get\_unannotated\_pool(), pick\_random\_frame() (Random); load\_next\_sequential\_frame(), AnnotatorTk.\_enter\_sequential\_mode() (Frame per frame); parse\_timecode\_csv(), \_parse\_frame\_value() (CSV); AnnotatorTk.\_load\_mining\_source()/\_adopt\_target\_list()/show\_target\_reason() (Mined frames); go\_to\_frame(), load\_next\_target(), AnnotatorTk.open\_source\_menu()/set\_nav\_mode(), \_scan\_videos\_recursive(); random, csv, re, cv2, AnnotationIndex.  
- **\[Plain\]**  
  - *How it works:* The tool lists every frame that can be labelled across all clips and removes those already done. A **Source** button lets you choose how the next frame is picked: *Random frames* shows a random remaining one each time you save or skip; *Frame per frame* stays in the video you are on and simply moves forward by a fixed number of frames each time (you choose the number, and the dialog tells you what it means in frames per second); *Load a time-code CSV* instead walks through a list of specific moments you provide in a CSV file (by frame number or by mm:ss time), jumping to the right video automatically and showing how far through the list you are; *Mined frames* walks a list the computer prepared for you, of the moments where it is getting things wrong or hesitating, and writes at the top of the window why each frame was picked. CSV files are kept in the project's timecodes/ folder, where a ready-to-edit example is created for you.  
...
### 3.1A Frame mining / active learning (BehaveAI\_mine\_frames.py)

- **\[Technical\]**  
  - *How it works:* Reads the `*_tracking.csv` files a batch pass leaves in output/ (the raw ones only — `_corrected`/`_stitched`/`_metric` are the same detections after further processing and would be counted twice), scores every frame by eight independent signals, and writes `mining_targets.csv` **into `mined_frames/`**, beside the decoded frames it points at, where the annotation tool finds it without a file dialog (a list still in output/ from before the move is picked up too). The list is the **resume point**: each row is deleted as its frame is saved, so an interrupted session leaves on disk exactly the work that is left and reopening it carries on rather than starting over. Reopening also prunes anything annotated by other means. A hand-written time-code CSV is never rewritten — without a frame column there is nothing to match on, and it is the user's own file. The signals, each a **stratum** with its own share of the annotation budget: **det\_gap** (0.20) — a track present at t−1 and t+1 with no detection at t; BoT-SORT keeps a lost track alive for `tracker_track_buffer` frames, so the tracker itself localises the detector's misses at no extra cost, and only the middle frame of each gap is proposed. **det\_missed** (0.08) — motion-only detections, i.e. a moving animal the static detector missed; the mirror case (static-only) is deliberately excluded because the motion stream only fires on movers (recall ≈ 0.08), so static-only describes most of the corpus and points at no defect. **det\_lowconf** (0.05) — winning confidence inside a hesitation band (0.10–0.35), bounded below by the project's own `primary_conf_thresh`. **pair\_unseen** (0.20) — a (static, motion) combination the human annotations never contain; the two taxonomies ask different questions (posture vs gait) rather than competing, so class inequality means nothing and only an *unobserved combination* is informative. The reference is the project's own labels (static and motion boxes of the same frame matched at IoU ≥ 0.5), so no plausibility table has to be invented; gated on the static class having at least 30 annotated boxes, without which the stratum reports label sparsity rather than model error. **flicker** (0.10) — a track whose class will not settle inside a 15-frame window. **rare\_class** (0.25) — any detection of a class with fewer than `mining_rare_max_count` annotated boxes (default 100). **attr** (0.04) — age/species crop verdicts under 0.60. **random** (0.08) — uniform frames, kept as the unbiased control and as coverage of the blind spots where the model emits no signal at all. Inside a stratum the ranking is **breadth-first across videos** — best frame of every video, then second-best of every video, and so on — because ranking by score alone lets the clip the detector struggles most on own the stratum, and the per-video cap does not help when the budget is smaller than cap × videos (500 frames over 33 videos never reaches a cap of 20). Measured on Horses_HERDWISE at budget 500: the least-represented video went from 1 frame to 7, and the first 100 rows went from 26 to 32 of the 33 videos. Selection then fills each quota in turn and round-robins the leftover budget; scores are never compared *across* strata (they are a gap length, a change count, a confidence margin and a uniform draw — sorting them together ranks the strata by numeric scale, not by interest). Anti-redundancy is enforced by a minimum spacing inside a video (default 3 s) and a per-video cap (default 20). Holdout videos are excluded by default and already-annotated frames always are. Output is interleaved round-robin so any prefix of the file remains a balanced sample.  
  - *Purpose:* Uniform random sampling reproduces the phenomenon, so it reproduces its imbalance: annotating faster yields proportionally more of what is already abundant. Mining spends the budget on the frames that would change the models. Its explicit limit: a class with **zero** annotated examples can never be proposed by a detector that has never seen it — those need a human ethogram sweep or the geometric proposals of `BehaveAI_complex_candidates.py`, and the run prints which classes are in that state.  
  - *Pre-caching (behaveai\_frame\_cache.py):* after writing the target list the miner **decodes every mined frame ahead of time** into `<project>/mined_frames/`, one `.static.jpg` + `.motion.jpg` + animation buffer + `.json` per frame (~32 MB per 4K frame with the buffer, ~7 MB without it — `mining_precache_animation`; the run states the estimate before starting), named `<video\_label>\_<frame>` — the annotation tool's own dataset naming. Serving one frame live costs a seek plus `frameWindow` sequential reads on a 4K clip over the SSHFS mount, i.e. seconds of interactive latency; the work is entirely predictable, so it is done once, in parallel (`mining_precache_workers`, default 4 — the wait is latency, not bandwidth, so concurrent requests amortise the round trips), with progress, rate and ETA logged. Frames cached by an earlier run are skipped, so re-running after an interruption is cheap. On save the cached image is **moved** into the dataset rather than re-encoded — but only when it is byte-for-byte what the dataset should hold: a frame carrying grey blocking rectangles (any grey box, or `static_blocks_motion`/`motion_blocks_static` actually applying to that stream) is written from the composited array instead, because moving it would silently drop the blocking that keeps the two detectors from cross-training. Whatever remains of the entry is deleted once the frame is saved. `mined_frames/` is git-ignored and fully regenerable. `--no-precache` writes only the list.  
  - *One implementation of the motion formula:* the false-colour image is not a view of the video, it **is** the training input (`annot_motion/images` holds these pixels, and the boxes refer to them), so `strategy`, `expA`, `expB`, `lum_weight`, `rgb_multipliers`, `chromatic_tail_only`, `frame_skip`, `motion_threshold` and `scale_factor` are not display preferences. A cache that drifted from the live path would feed the dataset images that no longer match what the annotator saw, with no error anywhere. `behaveai_frame_cache.compute_frame_data()` is therefore the **only** implementation: BehaveAI\_annotation.py's prefetch thread and its main loader both call it, and a test compares its output pixel-for-pixel against the pre-refactor code on a decoded video.  
  - *Launching it:* the **Mine frames** button in stage *2 – Annotate* of the launcher, placed above **Annotate** because that is the order the two are used in. Like every launcher tool it is run as `python BehaveAI_mine_frames.py <project_dir>`, so the project is accepted positionally as well as through `--project` (and through the `BEHAVEAI_PROJECT` variable the launcher also exports). The button passes no other argument: the size of a pass is the `mining_budget` setting, so the GUI has one place to change it rather than a flag the button would have to carry.  
  - *Parameters:* `--project` (or a positional project path), `--budget` (defaults to `mining_budget`), `--fps` (30, used for spacing and time-codes), `--min-spacing-s` (3.0), `--max-per-video` (20), `--quota name=fraction,…`, `--include-holdout`, `--seed`, `--out`, `--no-precache`, `--precache-workers`, `--no-animation-cache`. Reads `mining_budget`, `mining_max_per_video`, `mining_rare_max_count`, `mining_quota_overrides`, `mining_precache_workers`, `mining_precache_animation`, `clips_dir`, `val_frequency`, `primary_conf_thresh` and `tracker_track_buffer` from the INI; the stratum shares and thresholds are module constants.  
  - *Implementation:* mine(), load\_project(), tracking\_csvs(), load\_tracking\_csv(), observed\_pairs(), class\_counts(), annotated\_frames(), signal\_\* (one per stratum), collapse(), select(), interleave(); behaveai\_holdout.is\_holdout\_video, behaveai\_config. Tests in tests/test\_mine\_frames.py.  
  - *Note:* The **pair\_unseen** stratum is skipped for tracking CSVs written before the per-stream columns were filled by source (§6.7): those could file a static verdict under `primary_motion_class`, which is exactly what this stratum reads. The run says how many CSVs were skipped for that reason. Mining must also be run **after** a real training run — the uncertainties of an under-trained detector describe its own state, not the difficulty of the frames.  
- **\[Plain\]**  
  - *How it works:* After the computer has watched a batch of videos, this script goes back over what it recorded and picks out the moments where it struggled: an animal it lost track of, one only the movement view spotted, a behaviour it kept changing its mind about, a combination no human ever labelled, or a rare behaviour it thinks it glimpsed. It writes those moments to a list, sharing the places out between the different kinds of problem so one of them cannot take over, never proposing two moments from the same few seconds, and leaving a small share to plain random frames as a check. The annotation tool can then walk that list.  
  - *Purpose:* If frames are picked at random, most of them show grazing and standing, because that is what horses mostly do — so labelling faster mainly produces more grazing. This picks the frames that would actually teach the models something.  
  - *Parameters:* how many frames to propose, how far apart they must be, how many per video, and how to share them between the kinds of problem.
...
- **\[Technical\]**  
  - *How it works:* If best.pt exists for primary static and/or motion, models load at startup and pre-populate boxes on unannotated frames; in hierarchical mode secondary classifiers run on each crop. Results are editable/clearable. Overlapping predictions are reduced with NMS. A proposal whose primary confidence is below `UNCERTAIN_CONF` (0.35, matching the upper edge of the miner's hesitation band, §3.1A) gets an **orange outer marker**. Hand-drawn boxes and boxes reloaded from a saved annotation carry −1 in the confidence slot and are therefore never marked: the flag says "the model doubts this", not "this is doubtful".  
  - *Purpose:* Bootstrap labelling speed using the current models (active-learning loop), and put the eye on the proposals worth a second look instead of requiring every label to be read.  
  - *Parameters:* primary\_conf\_thresh, secondary\_conf\_thresh; UNCERTAIN\_CONF is a module constant, not an INI key.  
  - *Implementation:* ultralytics.YOLO, auto\_annotate\_local(), non\_max\_suppression(), iou(), draw\_boxes\_on\_image.\_is\_uncertain(); behaveai\_render.draw\_labeled\_detection(uncertain=…) — the output-video renderer never passes it, since on a finished video every box is the pipeline's own answer.  
- **\[Plain\]**  
  - *How it works:* If trained files exist, the tool pre-draws its best guesses on new frames; you accept, fix, or clear them. In the two-tier mode it also guesses the finer category. Guesses the model is unsure about get an orange outline, so you can see at a glance which ones to check; boxes you drew yourself are never outlined that way.  
  - *Purpose:* Speed up labelling by starting from the model's guesses, and make the shaky guesses obvious.  
...
- **\[Technical\]**  
  ...
  - *Parameters:* primary\_classifier, primary\_epochs, primary\_imgsz, secondary\_classifier, secondary\_epochs, secondary\_imgsz, train\_patience, val\_frequency (the split every model trains against), primary\_train\_overrides / secondary\_train\_overrides. **Image size governs training and inference together**: Ultralytics writes imgsz into the checkpoint and `predict()` reads it back, so the two can never drift apart. It is rounded to a multiple of 32. **Train overrides** pass arbitrary Ultralytics training arguments straight through to `model.train()` (e.g. `mosaic=0.0, scale=0.2`, whose defaults shrink already-marginal aerial targets further); they are validated against Ultralytics' own `DEFAULT_CFG_DICT`, coerced to the shipped default's type, and refused for keys the pipeline sets itself (imgsz/epochs/patience/data/project), with every rejection reported on the console and in the settings GUI at save time. The detectors and the crop classifiers get separate lists because the two tasks read different arguments. When motion\_disable\_color\_aug is on, the motion stream's colour rules (hsv\_\*, auto\_augment) override anything set here — the motion images encode movement as colour. On 3840 px drone frames the default 640 divides every animal by six, which is what bounds recall on the small ones — see §6.4 for the alternative (SAHI tiling) and the measured trade-off.  
...
- **\[Technical\]**  
  ...
  - *Implementation:* ultralytics.YOLO, iou(), record\_stream\_verdict(), per-frame loop in process\_video(); cv2, numpy.  
- **\[Plain\]**  
...
- **\[Technical\]**  
  - *How it works:* Per video, writes \<name\>\_tracking.csv and, when detection\_video\_enabled is on (off by default), \<name\>\_detected.mp4 (boxes, labels, track IDs, motion arrows, frame counter). The CSV carries the 12 base columns (frame,id,x,y,primary\_static\_class,primary\_static\_conf,primary\_motion\_class,primary\_motion\_conf,secondary\_static\_class,secondary\_static\_conf,secondary\_motion\_class,secondary\_motion\_conf), then the four bounding-box columns x1,y1,x2,y2 ((x,y) remains the box midpoint), then species\_class,species\_conf,age\_class,age\_conf, then **source**. Columns are only ever appended, so a reader of the first 12 (or 16, or 20) columns keeps working. The per-stream class columns are filled **by source**: `primary_static_*` carries the best static-detector verdict on that animal and `primary_motion_*` the best motion one, an empty class meaning that detector did not see it at all — which is what makes the pair informative to the activity budget's arbitration (§7), complex features (§6.10) and the frame miner (§3.1A). Detections are merged by proximity into one box per animal and the merge is winner-takes-all, so the surviving box comes from one stream; `source` names which, i.e. which image the crop classifiers ran on. (Before this, the two columns were filled from the merge's winner/displaced pair regardless of source: under `dominant_source = confidence` two static detections merging filed a static verdict under `primary_motion_class`, and every reader downstream trusted it. Tracking CSVs predating the fix are recognisable by the absence of the `source` column.) After all videos, the post-processing chain runs in order, each stage gated by its own switch: **drone motion correction** (§6.8, drone\_correction\_enabled) → **tracklet stitching** (§6.6, stitch\_enabled) → **metric geometry** (§6.9, metric\_enabled) → **interaction graph** (§6.10, interaction\_graph\_enabled; skipped with an explanatory message when metric\_enabled is off, because it measures in metres) → **complex-behaviour stage** (§6.12, complex\_classify\_enabled; a no-op for a project with no complex annotations and no trained model) → **activity budget** (§7, always). Each stage is wrapped so a failure is reported without aborting the rest.  
  - *Processing order:* `video_order` = `name` (default, alphabetical) or `random`, with `video_order_seed` (0 = a fresh shuffle each run, non-zero = pinned). A completed run is identical either way; the setting matters for a run that is **interrupted or read before it finishes**. The clips are named from their capture timestamp, so name order is also chronological and groups by site and by herd — a partial pass therefore covers the first sites only, and everything downstream inherits the bias, since the frame miner (§3.1A) scores whatever tracking CSVs happen to exist and the activity budget (§7) aggregates them. Random order makes any prefix of the run a representative sample of the corpus. The run prints the chosen order and a `[n/total]` line per video.  
  ...
  - *Parameters:* `video_order`, `video_order_seed`; the display style (§2.9) for the overlay; the stage switches above.  
...
| primary\_imgsz | 640 | Resolution the detectors train **and infer** at (stored in the checkpoint) |
| primary\_train\_overrides | *(empty)* | Extra `model.train()` args for the detectors, `key=value` comma-separated |
...
| secondary\_imgsz | 224 | Resolution the crop classifiers train and infer at |
| secondary\_train\_overrides | *(empty)* | Same, for the crop classifiers (secondary, species, age) |
...
└── <project_name>/
    ...
    │                                     (mining_targets.csv moved to mined_frames/, §3.1A)
