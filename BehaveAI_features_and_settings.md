...
**Document version: 4.2** — updated 2026-08-05. **Every model now shares one train/validation partition, and its unit is the whole video.** New §3.9; §3.8, §6.1 and the `val_frequency` row in §11 updated.

- **The crop classifiers were splitting per crop, not per video.** The two per-stream secondary models and the species/age models were handed their flat crop pool (`annot_<stream>_crop/`) as the dataset. Ultralytics, finding no `train/` subdirectory, ran its own `split_classify_dataset()`: **random per crop, unseeded, copied into a `<pool>_split` directory created with `exist_ok` and never cleaned.** Every retrain reshuffled into the previous run's files. Measured on the reference project before the fix: **1460 of 1647 static validation crops (88.6 %) and 309 of 338 motion ones (91.4 %) were the same files also present in training**, and classes deleted in the 2026-06 ethogram migration (`arched_neck`, `recumbent`, `Kneeling`) were still being trained on. **Secondary-classifier validation metrics produced before this version are not interpretable.**
- **Fix:** `behaveai_holdout.build_classification_split()` materialises `annot_<pool>_split/{train,val}/<class>/` using the same `is_holdout_video()` partition as the detectors, and that directory — never the pool — is what `maybe_retrain()` hands to Ultralytics. It rebuilds from scratch behind a fingerprint of every pooled filename, so deleted classes and crops cannot survive; it creates each class folder on both sides even when empty, so train and val agree on the class indices. `video_label_for_crop()` recovers a crop's source video from its filename.
- **Consequence for reporting:** the realised validation share is no longer the target. On the reference project (`val_frequency = 0.2`): detectors 436/109 frames per stream (20.0 %), static crops 3660/701 (16.1 %), motion crops 666/120 (15.3 %) — held-out videos carry fewer annotated individuals than average. Quote the *video* split (53/263, 20.2 %) as the design, and the frame/crop counts as what it yielded.
- **A per-video split cannot balance a rare class**, and the code now says so instead of moving crops across the boundary: it warns for a class with no training crop at all (unlearnable) and notes a class absent from the holdout (trained but never validated).
...
- **Tracker — BoT-SORT / ByteTrack** now the default, selectable via `tracker_type` (`botsort` | `bytetrack` | `kalman`) with camera-motion compensation (GMC) and two-tier matching; the legacy Kalman tracker is kept as an option. The Tracking settings tab shows the relevant parameters per choice.
- **Offline tracklet stitching** (`BehaveAI_stitch_tracklets.py`, `stitch_enabled`): re-links the causal tracker's tracklets into longer identities over the whole clip, on kinematics only. Runs after drone correction; K (group size) is a reported diagnostic, never a constraint.
- **Appearance Re-ID REMOVED** — **§6.6, §6.7, §6.8 (Re-ID, self-supervised fine-tuning, segmentation auto-training) and the Re-Identification parameter table in §11 are obsolete.** Appearance is uninformative at 15–50 m; long-gap recovery is now the stitching pass's job. `BehaveAI_reid.py`, `BehaveAI_reid_finetune.py`, `BehaveAI_segmentation.py` and the `model_reid/` project folder are gone (segmentation kept aside for possible posture/behaviour use). `timm` dependency dropped.
- **Evaluation** (`BehaveAI_evaluate_tracking.py`): HOTA/DetA/AssA/IDF1/MOTA/IDSW via TrackEval against a MOT-format ground truth.
- **Drone/telemetry code is now a plugin package** `behaveai_drone/` (drone_correction, flightlog, srt_telemetry, horizon_geometry, metric_geometry). The metric chain (pixels → ground-plane metres via flight-log `rel_alt` + gimbal pitch) is wired (`metric_enabled`).
- **Pipeline order** is now: `classify_track → drone_correction → stitching → metric_geometry → activity_budget` (the activity budget runs **last**, on the most-processed CSV per video).

**Document version: 3.1** — updated 2026-07-15. Removes the fission-fusion sub-grouping module (formerly §6.10, BehaveAI\_subgroups.py): group-level features (polarisation, cohesion, hull area, centroid speed, synchrony, elongation) and the heuristic group behaviours (stampede, trek, synchronised\_rest\_graze) are now computed directly over the **whole co-present herd per frame**, with no spatial partitioning into sub-groups. The sub-grouping method (per-frame proximity clustering with temporal debounce) is kept as a separate, unpublished exploratory script outside this codebase and is discussed as a future perspective rather than as an implemented method.

**Document version: 3.0** — updated 2026-06-23. Major release documenting the **HERDWISE multi-individual / complex-behaviour pipeline** built on top of per-individual tracking: bounding-box columns in the tracking CSV (§6.5), **drone motion correction** (§6.9), the **interaction graph** with dyadic + group features as the primary analysis output (§6.10), a dedicated **complex-behaviour annotation tool** (§6.11), a **supervised complex-behaviour model** with a scikit-learn baseline and optional LSTM/Transformer sequence models evaluated by-video (§6.12), and **heuristic + active-learning candidate proposal** (§6.13). Also: a **redesigned secondary-behaviour system** (shared pool + per-primary mapping, optional-per-box, two per-stream models — §2.1, §3.6, §3.8); a **GUI overhaul** (inline ⓘ parameter help, consistent theme, all hidden config params exposed — §2); auto-retrain without confirmation pop-ups and inter-training GPU-memory release (§6.1); a **Google-Sheets time-code template generator**; and the skip-frame shortcut moved from Tab to Ctrl+P (§3.6). New INI parameters are in §11.
...
- **Interaction graph** — dyadic + group (whole co-present herd) features aggregated into an R-igraph-ready directed graph (primary analysis output) → BehaveAI\_complex\_features.py (§6.10)  
...
*The Settings GUI was overhauled: every parameter carries an inline **ⓘ help marker** with a hover tooltip (text from BehaveAI\_settings\_help.py / PARAM\_HELP), a consistent **theme** is applied via apply\_theme(), and all previously hidden/code-only config keys are now editable. Settings are organised into tabs: **Model structure, Video paths, Data augmentation, Motion strategy, Model type, Tracking** (now also hosting the Drone-motion-correction group), **Re-Identification, Interaction, Complex Behaviours, Display Settings, Activity Budget**.*
...
- **\[Technical\]**  
  ...
  - *Parameters:* val\_frequency (fraction of videos held out; the realised frame/crop percentage differs from it, since videos carry unequal numbers of annotations).  
...
- **\[Technical\]**  
  ...
  - *Parameters:* primary\_classifier, primary\_epochs, secondary\_classifier, secondary\_epochs, val\_frequency (the split every model trains against), image sizes (primary native, secondary 224).  
  - *Implementation:* ultralytics.YOLO(...).train(), count\_images\_in\_dataset(), move\_to\_expected(), behaveai\_holdout.build\_classification\_split(), torch.cuda.empty\_cache()/synchronize() between trainings; shutil, glob, configparser.  
- **\[Plain\]**  
...
> **Optional secondary via a `__none__` class (2026-07-15).** The secondary classifier now includes a reserved **`__none__`** class so it can answer *"no secondary"* instead of being forced to emit one. In the annotation tool, secondary-eligible primaries default to **none** (shown as the first *none* button; hotkey **`n`**, a reserved key); leaving a box untouched writes a negative crop to `annot_<stream>_crop/__none__/`. Training picks up `__none__` as an extra pooled class; at inference `__none__` competes in the arg-max and, when it wins, the box gets no secondary (this replaced the old confidence-threshold-only reject). `BehaveAI_inspect_dataset.py` now writes crops in the same **pooled** `annot_<stream>_crop/<secondary|__none__>/` layout, routed to the primary's own stream (it previously wrote a per-primary layout into both streams).
...
- **\[Technical\]**  
  ...
  - *Purpose:* Remove drone-induced apparent motion so per-horse kinematics (and everything downstream — interactions) reflect real movement.  
...
└── \<project\_name\>/
    ...
    │                                      merge\_suggestions.txt, feature\_importances.txt (§6.12)
...
- **\[Technical\]**  
  ...
  - *Annotation:* random-frame sampling or CSV-driven time-code navigation (targeted annotation of specific frames/events, by integer frame or mm:ss), dual-stream YOLO-format boxes, optional hierarchical secondary crops, optional cross-stream grey masking, whole-video train/val split \= val\_frequency (all models, detectors and crop classifiers alike).  
  ...
  - *Detection & tracking:* optional SAHI sliced inference on native-resolution tiles (with a matching tiled dataset retrain) to recover small/distant subjects → dual detection → centroid/IOU merge (dominant\_source rule) → secondary classification → multi-object tracking by **BoT-SORT / ByteTrack** (Ultralytics; global camera-motion compensation + two-tier high/low-score association), selectable (tracker\_type) back to the legacy constant-velocity Kalman + Hungarian tracker.  
  - *Identity (offline):* the causal tracker's short tracklets are re-linked over the whole clip by a non-causal **kinematic stitching** pass — hard constraints of temporal non-overlap and a maximum plausible speed, a normalised gap cost, and a dummy-augmented linear assignment; the known group size K is a reported diagnostic, never a constraint. Appearance / Re-ID is deliberately not used (uninformative on a ~60×35 px horse at 15–50 m).  
  - *Evaluation:* tracking is scored against a hand-annotated MOT-format ground truth with **HOTA — reported as DetA and AssA separately** — plus IDF1, MOTA, ID switches and fragmentation (TrackEval), so detection and association gains can be attributed independently.  
  ...
  - *Analysis (multi-individual, HERDWISE):* drone-induced apparent motion is removed by background optical-flow stabilisation (RANSAC affine/homography on horse-masked frames) and velocities recomputed from corrected, smoothed centroids; dyadic (distance, approach rate, velocity-cosine, contact) and group (polarisation, cohesion, hull area, synchrony, elongation — computed over the whole co-present herd per frame) features — all normalised by a robust adult reference body length, foals excluded — are aggregated into a directed interaction graph (networkx; R-igraph-ready edges/nodes CSVs). Multi-individual behaviours are hand-annotated, then classified by a by-video-evaluated model (scikit-learn baseline or optional LSTM/Transformer) that fuses the kinematic/graph features with the per-individual YOLO labels; heuristic + active-learning candidate proposal accelerates annotation. Positions are optionally projected to **ground-plane metres** from flight-log telemetry (relative altitude + gimbal pitch + focal length; `behaveai_drone` plugin), giving real-world inter-individual distances (m) and speeds (m/s); without a flight log the pipeline stays in image space.  
