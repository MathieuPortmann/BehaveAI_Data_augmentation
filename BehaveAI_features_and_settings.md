...
**Version 5.1 — 2026-08-14.** This document describes the pipeline **as it currently stands**. It is a reference, not a change log: anything removed from the code is removed from here.
...
- **\[Technical\]**  
  ...
  - *Parameters:* `--project`, `--budget` (300), `--fps` (30, used for spacing and time-codes), `--min-spacing-s` (3.0), `--max-per-video` (20), `--quota name=fraction,…`, `--include-holdout`, `--seed`, `--out`. Reads `val_frequency`, `primary_conf_thresh` and `tracker_track_buffer` from the INI; the stratum shares and thresholds are module constants.  
