...
- **\[Technical\]**  
  - *How it works:* Reads the `*_tracking.csv` files a batch pass leaves in output/ (the raw ones only — `_corrected`/`_stitched`/`_metric` are the same detections after further processing and would be counted twice), scores every frame by eight independent signals, and writes `mining_targets.csv` for the annotation tool's Mined-frames mode. The signals, each a **stratum** with its own share of the annotation budget: **det\_gap** (0.20) — a track present at t−1 and t+1 with no detection at t; BoT-SORT keeps a lost track alive for `tracker_track_buffer` frames, so the tracker itself localises the detector's misses at no extra cost, and only the middle frame of each gap is proposed. **det\_missed** (0.10) — motion-only detections, i.e. a moving animal the static detector missed; the mirror case (static-only) is deliberately excluded because the motion stream only fires on movers (recall ≈ 0.08), so static-only describes most of the corpus and points at no defect. **det\_lowconf** (0.05) — winning confidence inside a hesitation band (0.10–0.35), bounded below by the project's own `primary_conf_thresh`. **pair\_unseen** (0.25) — a (static, motion) combination the human annotations never contain; the two taxonomies ask different questions (posture vs gait) rather than competing, so class inequality means nothing and only an *unobserved combination* is informative. The reference is the project's own labels (static and motion boxes of the same frame matched at IoU ≥ 0.5), so no plausibility table has to be invented; gated on the static class having at least 30 annotated boxes, without which the stratum reports label sparsity rather than model error. **flicker** (0.15) — a track whose class will not settle inside a 15-frame window. **rare\_class** (0.10) — any detection of a class with fewer than 50 annotated boxes. **attr** (0.05) — age/species crop verdicts under 0.60. **random** (0.10) — uniform frames, kept as the unbiased control and as coverage of the blind spots where the model emits no signal at all. Selection fills each quota in turn, then round-robins the leftover budget; scores are never compared *across* strata (they are a gap length, a change count, a confidence margin and a uniform draw — sorting them together ranks the strata by numeric scale, not by interest). Anti-redundancy is enforced by a minimum spacing inside a video (default 3 s) and a per-video cap (default 20). Holdout videos are excluded by default and already-annotated frames always are. Output is interleaved round-robin so any prefix of the file remains a balanced sample.  
