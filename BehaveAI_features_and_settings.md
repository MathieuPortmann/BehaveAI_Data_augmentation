...
- **\[Technical\]**  
  - *How it works:* Sets val\_frequency, primary\_classifier (base detector, e.g. yolo11s.pt; yolo8/11/26 in n/s/m/l), primary\_epochs, secondary\_classifier (e.g. yolo11s-cls.pt), secondary\_epochs, train\_patience (early-stopping patience), use\_ncnn, primary\_conf\_thresh, secondary\_conf\_thresh, and dominant\_source (confidence/motion/static). The SAHI sliced-inference group (sahi\_enabled\_static/motion and the slice/overlap/post-process keys, §6.4) lives here too.  
...
- **\[Technical\]**  
  - *How it works:* **Display:** base line\_thickness / font\_size plus the shared box-rendering style used by both the output video and the annotation tool (box\_line\_scale, box\_font\_scale, adaptive\_box\_scaling and its coefficient/min/max pairs, label\_bg\_mode / label\_bg\_opacity / label\_bg\_color, halo\_thickness / halo\_color, show\_species, show\_age, buttons\_per\_row). **Activity budget:** ab\_min\_presence\_seconds, ab\_min\_classified\_frames, ab\_edge\_margin\_px, ab\_analysis\_duration\_s, ab\_group\_type\_separator, ab\_group\_type\_field\_index (zero-based filename field for group type).  
...
- **\[Technical\]**  
  - *How it works:* The online tracker is causal: once it cuts a track or swaps an id it never revisits the decision. The video, however, is on disk, so this non-causal pass reads the whole clip and re-links the short, reliable tracklets into longer identities — **purely on kinematics, no appearance**. It reads a tracking CSV (preferring the drone-corrected one, whose x\_corrected/y\_corrected live in a single stabilised reference frame comparable across the whole clip), groups rows into tracklets by id, discards tracklets shorter than stitch\_min\_tracklet\_len, and links compatible end→start pairs. Two **hard constraints** make the problem safe: two tracklets overlapping in time are different animals (never linked), and an implied speed above stitch\_max\_speed\_px\_per\_frame is physically impossible (never linked). The **soft cost** is the normalised gap between where tracklet A is heading and where B starts, in \[0,1\]; a link is accepted below stitch\_max\_link\_cost, via a dummy-augmented linear assignment so leaving a tracklet unlinked is always an option. stitch\_quality\_gate refuses to link across frames the drone correction flagged as unreliable. The resulting group size K is **reported as a diagnostic, never imposed as a constraint**. Output: \<video\>\_tracking\_stitched.csv.  
...
- **\[Technical\]**  
  ...
  - *Parameters:* metric\_enabled, metric\_focal\_len\_mm, metric\_sensor\_width\_mm, metric\_horizon\_margin\_px, metric\_roll\_max\_deg, metric\_scale\_tolerance, metric\_assumed\_body\_length\_m (see §11). Requires a `<video>.flightlog.csv` sidecar.  
...
| stitch\_min\_tracklet\_len | 2 | Tracklets shorter than this (frames) are dropped, not linked |
| stitch\_max\_speed\_px\_per\_frame | 60 | Hard gate: a link implying more than this speed is impossible |
| stitch\_max\_link\_cost | 0.5 | Soft gate: normalised gap cost \[0,1\] above which a link is refused |
...
| motion\_disable\_color\_aug | true | Forbid colour transforms on motion images (§5.3) |
...
- **\[Technical\]**  
  ...
  - *Identity (offline):* the causal tracker's short tracklets are re-linked over the whole clip by a non-causal **kinematic stitching** pass — hard constraints of temporal non-overlap and a maximum plausible speed, a normalised gap cost, and a dummy-augmented linear assignment; the known group size K is a reported diagnostic, never a constraint.  
